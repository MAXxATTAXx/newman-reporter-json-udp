...
class JsonUDPReporter {
    ...
    start(err, e) {
        ...
        if (this.reporterOptions.jsonUdpRequestHeaders) {
            this.reporterOptions.jsonUdpRequestHeaders = this.reporterOptions.jsonUdpRequestHeaders.split(',');
        } else {
            this.reporterOptions.jsonUdpRequestHeaders = [];
        }
    ...
    request(err, e) {
        ...
        let reqHeaders = this.extractHeaders(e.request.headers, this.reporterOptions.jsonUdpRequestHeaders);

        Object.assign(this.currItem, {
            status, code, responseTime, responseSize, responseHeaders: resHeaders, requestHeaders: reqHeaders, assertions: {
                failed: 0,
                failed_names: [],
                skipped: 0,
                skipped_names: [],
